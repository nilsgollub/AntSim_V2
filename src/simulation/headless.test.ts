...
describe('headless harness — golden (behaviour pinned)', () => {
    ...
    // Re-pinned after merging the nest visual redesign into main: digging recruitment
    // now opens the second chamber earlier (extraChambers 2 already at t1000) and the
    // queenEnergy delta at t2500 is one feed cycle higher; sugar reserves shift by ~10.
    it('matches the pinned snapshot at 1000 ticks', () => {
        expect(runHeadless(12345, 1000)).toEqual({
            ...
            brood: 23, larvae: 16, sugar: 673, protein: 238,
    ...
    it('matches the pinned snapshot at 2500 ticks', () => {
        expect(runHeadless(12345, 2500)).toEqual({
            ...
            brood: 29, larvae: 15, sugar: 711, protein: 204,
